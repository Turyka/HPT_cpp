...
public:
	...
	FixStorage(FixStorage &&f) {
		size = f.size;
		array = f.array;
		f.array = nullptr;
	}

	FixStorage& operator=(FixStorage &&f) {
		if(this == &f)
			return *this;
...
float interpolateFunc(float start, float end, float q, std::function<float(float)> fv) {
...
//	100
	...
	for(i = 0, f = 0.0; i <= 20; i++, f += 0.05) {
		...
		//std::cout << interpolateFunc(0, 1, f, lsqr) << ", " << interpolateFunc(0, 1, f, lsqrt) << std::endl;
...
//	1, 1
	...
	std::thread t1([&res, &_mutex, number]() { 
		_mutex.lock();
		res = fibo(number);
		_mutex.unlock();
				   });
	std::thread t2([&_mutex]() { idoMeres(_mutex); });
